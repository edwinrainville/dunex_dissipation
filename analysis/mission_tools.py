...
def compute_individual_waves(x_locations, y_locations, eta, time,
...
def closest_awac_mean_wave_period(mission_time, awac_file):
    """
    Find the closest AWAC mean wave period
    ...
    awac_mean_wave_period = np.interp(mission_time,
                        awac_data['time'][:],
                        awac_data['waveTm'][:])

    awac_data.close()
    return awac_mean_wave_period

def closest_awac_mean_wave_direction(mission_time, awac_file):
    """
    Find the closest AWAC mean wave direction
    ...
    awac_mean_wave_direction = np.interp(mission_time,
                        awac_data['time'][:],
                        awac_data['waveMeanDirection'][:])

    awac_data.close()
    return awac_mean_wave_direction
...
def save_case_study_data(fname, h_rms_profile, cross_shore_bin_centers, 
        ...
        """
        Save all parameters and values for each case study for further
        analysis.
        ...
        fname : str
            filename to save to
        h_rms_profile : list
            list of rms wave heights in the cross shore
        cross_shore_bin_centers : cross shore location of 
            _description_
        Hs_offshore_inmission : _type_
            _description_
        x_surfzone : _type_
            _description_
        Tm_offshore_inmission : _type_
            _description_
        mean_dir_offshore_inmission : _type_
            _description_
        wave_frequency : _type_
            _description_
        wave_energy_offshore_inmission : _type_
            _description_
        """
    
        data = {'h_rms':h_rms_profile, 
            'cross_shore_bin_centers':cross_shore_bin_centers,
            'Hs_offshore':Hs_offshore_inmission, 
            'x_surfzone':x_surfzone,
            'Tm_offshore':Tm_offshore_inmission}
